...
export const playlistsRouter = createTRPCRouter({
  ...
  removeVideo: protectedProcedure
    .input(z.object({ playlistId: z.string().uuid(), videoId: z.string() }))
    .mutation(async ({ input, ctx }) => {
  ...
  getHistory: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const viewerVideoView = db.$with("viewer_video_views").as(
        db
          .select({
      ...
      const data = await db
        .with(viewerVideoView)
        .select({
          ...getTableColumns(videos),
          user: users,
          viewedAt: viewerVideoView.viewedAt,
          viewCount: db.$count(videoViews, eq(videoViews.videoId, videos.id)),
          likeCount: db.$count(
            videoReactions,
            and(
        ...
        .from(videos)
        .innerJoin(users, eq(videos.userId, users.id))
        .innerJoin(viewerVideoView, eq(videos.id, viewerVideoView.videoId))
        .where(
          and(
            eq(videos.visibility, "public"),
            cursor
              ? or(
                  lt(viewerVideoView.viewedAt, cursor.viewedAt),
                  and(
                    eq(viewerVideoView.viewedAt, cursor.viewedAt),
                    lt(videos.id, cursor.id),
                  ),
                )
              : undefined,
        ...
        .orderBy(desc(viewerVideoView.viewedAt), desc(videos.id))
        .limit(limit + 1);
