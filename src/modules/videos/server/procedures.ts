...
export const videoRouter = createTRPCRouter({
  getManySubscribed: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const viewerSubscribed = db.$with("viewer_subscriptions").as(
        db
          .select({
      ...
      const data = await db
        ...
        .innerJoin(viewerSubscribed, eq(viewerSubscribed.userId, videos.userId))
        .where(
          and(
  ...
  revalidate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // const [cleanedVideo] = await db
      //   .update(videos)
      //   .set({
      //     muxPlaybackId: null,
      //     muxAssetId: null,
      //   })
      //   .where(and(eq(videos.id, input.id), eq(videoReactions.userId, userId)))
      //   .returning();
  ...
  remove: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (removeVideo.thumbnailKey) {
        utapi.deleteFiles(removeVideo.thumbnailKey);
      }

      if (removeVideo.previewKey) {
        utapi.deleteFiles(removeVideo.previewKey);
      }
      return removeVideo;
    }),
